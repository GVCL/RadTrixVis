...
const ForceEdgeBundling = function () {
	var data_nodes = {}, // {'nodeid':{'x':,'y':},..}
		...
		K = 0.1, // global bundling constant controlling edge stiffness :default = 0.1
		...
		I_initial = 90, // init. number of iterations for cycle :default=90, lower value -> straighter lines
		I_rate = 0.6666667, // rate at which iteration number decreases i.e. 2/3
		compatibility_threshold = 0.4, // the higher the value, we will get more n more straight lines :default=0.6
		eps = 1e-6;
	...
	function compute_divided_edge_length(e_idx) {
		...
		for (var i = 1; i < subdivision_points_for_edge[e_idx]["points"].length; i++) {
			var segment_length = euclidean_distance(subdivision_points_for_edge[e_idx]["points"][i], subdivision_points_for_edge[e_idx]["points"][i - 1]);
	...
	function initialize_edge_subdivisions() {
		for (var i = 0; i < data_edges.length; i++) {
			if (P_initial === 1) {
				subdivision_points_for_edge[i] = {"points": [], "info": {}}; //0 subdivisions
				subdivision_points_for_edge[i]["info"]["target_color"] = data_edges[i].target_color;
				subdivision_points_for_edge[i]["info"]["source"] = data_edges[i].source;
			} else {
				subdivision_points_for_edge[i]= {"points": [], "info": []};
				subdivision_points_for_edge[i]["info"]["target_color"] = data_edges[i].target_color;
				subdivision_points_for_edge[i]["info"]["source"] = data_edges[i].source;
				subdivision_points_for_edge[i]["points"].push(data_nodes[data_edges[i].source]);
				subdivision_points_for_edge[i]["points"].push(data_nodes[data_edges[i].target]);
			}
			// console.log("target color defined");
			// if (P_initial === 1) {
			// 	subdivision_points_for_edge[i]["points"] = []; //0 subdivisions
			// } else {
			// 	subdivision_points_for_edge[i]["points"] = [];
			// 	subdivision_points_for_edge[i]["points"].push(data_nodes[data_edges[i].source]);
			// 	subdivision_points_for_edge[i]["points"].push(data_nodes[data_edges[i].target]);
			// }
		}
	...
	function apply_spring_force(e_idx, i, kP) {
		var prev = subdivision_points_for_edge[e_idx]["points"][i - 1];
		var next = subdivision_points_for_edge[e_idx]["points"][i + 1];
		var crnt = subdivision_points_for_edge[e_idx]["points"][i];
	...
	function apply_electrostatic_force(e_idx, i) {
		...
		for (var oe = 0; oe < compatible_edges_list.length; oe++) {
			var force = {
				'x': subdivision_points_for_edge[compatible_edges_list[oe]]["points"][i].x - subdivision_points_for_edge[e_idx]["points"][i].x,
				'y': subdivision_points_for_edge[compatible_edges_list[oe]]["points"][i].y - subdivision_points_for_edge[e_idx]["points"][i].y
			};

			if ((Math.abs(force.x) > eps) || (Math.abs(force.y) > eps)) {
				var diff = (1 / Math.pow(custom_edge_length({
					'source': subdivision_points_for_edge[compatible_edges_list[oe]]["points"][i],
					'target': subdivision_points_for_edge[e_idx]["points"][i]
				}), 1));
	...
	function update_edge_divisions(P) {
		for (var e_idx = 0; e_idx < data_edges.length; e_idx++) {
			if (P === 1) {
				subdivision_points_for_edge[e_idx]["points"].push(data_nodes[data_edges[e_idx].source]); // source
				subdivision_points_for_edge[e_idx]["points"].push(edge_midpoint(data_edges[e_idx])); // mid point
				subdivision_points_for_edge[e_idx]["points"].push(data_nodes[data_edges[e_idx].target]); // target
			} else {
				...
				for (var i = 1; i < subdivision_points_for_edge[e_idx]["points"].length; i++) {
					var old_segment_length = euclidean_distance(subdivision_points_for_edge[e_idx]["points"][i], subdivision_points_for_edge[e_idx]["points"][i - 1]);

					while (old_segment_length > current_segment_length) {
						var percent_position = current_segment_length / old_segment_length;
						var new_subdivision_point_x = subdivision_points_for_edge[e_idx]["points"][i - 1].x;
						var new_subdivision_point_y = subdivision_points_for_edge[e_idx]["points"][i - 1].y;

						new_subdivision_point_x += percent_position * (subdivision_points_for_edge[e_idx]["points"][i].x - subdivision_points_for_edge[e_idx]["points"][i - 1].x);
						new_subdivision_point_y += percent_position * (subdivision_points_for_edge[e_idx]["points"][i].y - subdivision_points_for_edge[e_idx]["points"][i - 1].y);
				...
				subdivision_points_for_edge[e_idx]["points"] = new_subdivision_points;
			}
		}
	}
		...
		for (var cycle = 0; cycle < C; cycle++) {
			for (var iteration = 0; iteration < I; iteration++) {
				...
				for (var e = 0; e < data_edges.length; e++) {
					for (var i = 0; i < P + 1; i++) {
						subdivision_points_for_edge[e]["points"][i].x += forces[e][i].x;
						subdivision_points_for_edge[e]["points"][i].y += forces[e][i].y;
					}
				}
			}
